...
const BUILD_TOKEN = 'dbg-20260328-01';
...
if (typeof require === 'function') {
  try {
    ...
    if (uxp && uxp.storage && uxp.storage.localFileSystem) {
      ...
      ({ runBatch } = require('./src/core/batchController'));
...
async function handleRun() {
  ...
  try {
    ...
    await runBatch(composeRunPayload(validation.actions), {
      onPreflight: (result) => {
        if (!result.ok) {
      ...
      onDebugLog: (message) => updateLogUI({ kind: 'DEBUG', fileName: '[DEBUG]', status: 'DEBUG', detail: message }),
