...
const clipboard = uxp && uxp.clipboard ? uxp.clipboard : null;
const { app } = require('photoshop');
const { runBatch, closeAllOpenDocsWithoutSaving }   = require('./src/core/batchController');
const { LOG_STATUS } = require('./src/constants/logStatus');
...
$('sortBy').addEventListener('change', (e) => { state.sortBy = e.target.value; });
...
$('btnRefreshActions').addEventListener('click', async () => {
  await refreshActionCatalog({ preserve: false, clearSelections: true });
});
