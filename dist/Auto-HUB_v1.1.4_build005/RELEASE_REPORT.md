# Auto-HUB v1.1.4 / build 005 릴리즈 보고서

릴리즈 날짜: 2026-06-20

## 패치 범위

앱 버전 `v1.1.4`를 유지하고 내부 빌드만 `build 005`로 올린 재패키징 설치본이다. 빌드 산출물 폴더를 `merged/`에서 `dist/`로 정리하고, 빌드/릴리즈 문서와 버전 토큰을 새 빌드 기준으로 동기화했다. 패널 런타임 기능 로직 변경은 없다.

## 확인 가능한 작업 범위

- Git commit: 미커밋 작업 범위
- Git tag: 미생성
- 변경 범위: `index.js`(빌드 토큰), `docs/`, `CLAUDE.md`, `dist/Auto-HUB_v1.1.4_build005/`
- 패키지 폴더: `dist/Auto-HUB_v1.1.4_build005`
- zip 파일: `dist/Auto-HUB_v1.1.4_build005.zip`
...
- 2026-06-20: 내부 빌드를 `build 005`로 올렸다. 앱 버전 `v1.1.4`는 유지한다.
- 2026-06-20: 빌드 산출물 폴더를 `merged/`에서 `dist/`로 변경하고 빌드 가이드/템플릿/보고서 경로를 정리했다.
...
- (없음) 이번 빌드는 재패키징 빌드로 신규 버그 수정 없음. 직전 build004까지의 버그 수정 이력은 아래를 유지한다.
...
- UXP Developer Tool에서 추가할 폴더: `dist/Auto-HUB_v1.1.4_build005`
- 기존 패널을 닫고 다시 열어야 하는지: 예, 기존 로드 패널을 reload 또는 remove 후 다시 add
- 기존 Action preset 유지 여부: 유지. 사용자 preset/settings 삭제 작업 없음
- 수동 제거가 필요한 이전 패키지: 없음. 다만 테스트 시 UXP Developer Tool이 루트와 이전 `dist/*` 중 어느 폴더를 로드 중인지 확인 필요
...
- 앱 버전/빌드 번호 확인: `manifest.json` `1.1.4`, panel title `Auto-HUB v1.1.4`, `BUILD_TOKEN` `v1.1.4-build005`, `packageBuildId` `build005`
