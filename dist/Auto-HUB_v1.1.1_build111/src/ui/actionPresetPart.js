...
function createActionPresetPart({
  ...
  function debugLog(...args) {
    ...
    try {
      onDebugEvent({
        ...
        event: 'message',
        severity: 'debug',
        display: args.map(item => typeof item === 'string' ? item : '[object]').join(' '),
        data: { args }
      });
    } catch (_) {}
  }
  ...
  function hasSlotSelection(slot) {
    return !!(sanitizeValue(slot && slot.setName) || sanitizeValue(slot && slot.actionName));
  }

  function isFavoriteSlot(slot) {
    return !!(slot && slot.enabled !== false && hasSlotSelection(slot));
  }
  ...
  function readCatalog() {
    ...
    if (sets.length === 0) {
      ...
      debugLog('[Auto-HUB] readCatalog empty:', state.lastReadDiag);
    } else {
      state.lastReadDiag = { decodedSets: sets.length };
      if (isDebugEnabled()) {
        const firstName = typeof sets[0].name === 'string' ? sets[0].name : '(non-string)';
        debugLog('[Auto-HUB] readCatalog sets=' + sets.length + ' first=' + firstName);
        ...
        if (!state.actionSetProbed) {
          ...
          debugLog('[Auto-HUB] ActionSet probe:', probe);
        }
      }
    }
  ...
  function render() {
    ...
    state.slots.forEach((slot, index) => {
      if (!hasSlotSelection(slot)) emptyEntries.push({ slot, index });
    ...
    const hasFavorite = enabledEntries.some(({ slot }) => isFavoriteSlot(slot));
    let mainEntries = enabledEntries;
    if (!hasFavorite) {
      mainEntries = emptyEntries.length > 0 ? [emptyEntries[0]] : [{ slot: state.slots[0], index: 0 }];
    }

    mainEntries.forEach(({ slot, index }) => {
      list.appendChild(buildSlotRow(slot, index));
    });

    if (hasFavorite && disabledEntries.length > 0) {
  ...
  async function refreshSetList() {
    ...
    try {
      ...
      debugLog('[Auto-HUB] set list refresh, count=' + next.length);
    } catch (error) {
      onProblem(error);
    } finally {
  ...
  async function refreshActionsForSet(setName) {
    ...
    try {
      ...
      debugLog('[Auto-HUB] partial refresh set=' + setName + ' actions=' + actionNames.length);
  ...
  function addEmptySlot() {
    ...
    if (!state.slots.some(isFavoriteSlot)) {
      if (!state.slots.some(slot => !hasSlotSelection(slot))) {
  ...
  async function init(ids) {
    ...
    debugLog('[Auto-HUB] init done, catalog sets =', state.catalogCommitted.length);
    if (isUxpRuntime && state.catalogCommitted.length === 0) {
      ...
      setTimeout(async () => {
        ...
        debugLog('[Auto-HUB] retry catalog sets =', state.catalogCommitted.length);
        if (state.catalogCommitted.length === 0) {
  ...
  async function restoreView() {
    ...
    state.lastRefreshAt = Date.now();
    await refreshCatalog({ preserve: true });
    debugLog('[Auto-HUB] restoreView done, catalog sets =', state.catalogCommitted.length);
  }
